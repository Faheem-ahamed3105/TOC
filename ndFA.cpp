...
int main()
{
	...
	{
		for(j=0;i<num_symbols;j++)
		{
			printf("How many transition from state %dfor the input%d:",i,symbols[j]);
			scanf("%d",&n);
			for(k=0;k<n;k++)
			{
				printf("Enter the transition %d form state %d for the input%d:",k+1,symbols[j]);
                scanf("%d",&mat[i][j][k]);
			}
		}
	}
